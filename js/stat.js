...
var showHistogram = function(ctx, index, name, height, color) {
  ctx.fillStyle = 'black';
...
var maxTime = 0;
var findMaxTime = function(arrRoundTime) {
...
for (var i = 0; i < arrTime.length; i++) {
  ...
  if (arrName[i] == "Вы") {
    showHistogram(ctx, i, arrName[i], height, 'rgba(255, 0, 0, 1)');
  } else {
  var randomNumber = Math.round(Math.random() * (100 - 1) + 1);
  var randomBlue = "hsl(245, " + randomNumber + "%, 50%)";
  showHistogram(ctx, i, arrName[i], height, randomBlue);
  }
}
}
